...
const CallsPage = () => {
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'call_records' }, () => { loadDbCalls(); })
      .subscribe();
                ...
                <div className="bg-card border border-border rounded-xl p-4 sm:p-5 shadow-sm">
                  <h3 className="font-semibold text-foreground mb-3 flex items-center gap-2">
                    ...
                    {selectedCall.audioUrl && (
                      <button className="text-xs text-primary hover:underline flex items-center gap-1 ml-auto"><Play size={10} /> Reproducir</button>
                    )}
                  </h3>
            ...
            <div className="space-y-4">
              <div className="bg-card border border-border rounded-xl p-4 sm:p-5 shadow-sm">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {Object.keys(selectedCall.extractedData).length === 0 && (<p className="text-muted-foreground text-xs">Sin datos extraídos</p>)}
                </div>
              </div>
              ...
              {selectedCall.extractedData.objections && selectedCall.extractedData.objections.length > 0 && (
                <div className="bg-card border border-border rounded-xl p-4 sm:p-5 shadow-sm">
                  ...
                  <ul className="space-y-2">
                    {selectedCall.extractedData.objections.map((o, i) => (<li key={i} className="text-sm text-muted-foreground">{o}</li>))}
                  </ul>
                </div>
              )}
            </div>
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <div className="flex-1 flex items-center gap-2 bg-card border border-border rounded-lg px-3 py-2">
          ...
          <input type="text" placeholder="Buscar llamadas..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="bg-transparent text-sm outline-none flex-1 text-foreground placeholder:text-muted-foreground" />
        </div>
          ...
          filtered.map(call => {
            ...
            return (
              <button
                ...
                className="w-full flex items-center gap-3 sm:gap-4 px-4 py-3 border-b border-border last:border-b-0 hover:bg-secondary/30 transition-colors text-left"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-muted-foreground truncate">{call.agentName} · {format(call.startedAt, "d MMM HH:mm", { locale: es })}</p>
                </div>
                <div className="text-right shrink-0 hidden sm:block">
                  <p className="text-sm text-foreground">{formatDuration(call.duration)}</p>
                  <p className={`text-xs ${cfg.className} px-1.5 py-0.5 rounded-full`}>{cfg.label}</p>
                </div>
