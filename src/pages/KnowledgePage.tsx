...
const KnowledgePage = () => {
  ...
  const handleSyncAll = async () => {
    ...
    try {
      const { data: tenantId } = await supabase.rpc('get_user_tenant_id', { _user_id: user.id });
      // Try to enqueue async — button unblocks immediately, worker drains via cron.
      let enqueued = false;
      try {
        ...
        if (!enqErr && jobRow?.id) {
          enqueued = true;
          // Fire-and-forget kick so the worker drains without waiting for cron.
          supabase.functions.invoke('background-job-worker', { body: {} }).catch(() => {});
          toast.success('Sincronización encolada. Se ejecutará en segundo plano.');
          fetchArticles();
        }
      } catch (e) {
        console.warn('[KB] enqueue failed, falling back to direct sync:', e);
      }
      if (!enqueued) {
