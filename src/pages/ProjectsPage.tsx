          ...
          {view === 'board' ? (
                  ...
                  <div key={status} className="flex flex-col">
                    ...
                    <div className="space-y-2 overflow-y-auto scrollbar-thin">
                      {colTasks.map(task => {
                        ...
                        return (
                          <div key={task.id} className="bg-card border border-border rounded-lg p-3 shadow-sm">
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5">
                                <div className="w-5 h-5 rounded-full bg-secondary flex items-center justify-center text-[10px] font-bold text-secondary-foreground">{task.assignee.split(' ').map(n => n[0]).join('')}</div>
            ...
            <div className="space-y-2">
              {projectTasks.map(task => {
                ...
                return (
                  <div key={task.id} className="bg-card border border-border rounded-lg p-3 flex items-center gap-3 sm:gap-4">
                    <div className={`w-2 h-2 rounded-full shrink-0 ${sConfig.dotClass}`} />
                    ...
                    {task.dueDate && <span className="text-xs text-muted-foreground hidden sm:block">{format(task.dueDate, 'd MMM', { locale: es })}</span>}
                    <span className="text-xs text-muted-foreground hidden sm:block">{sConfig.label}</span>
                  </div>
                );
              })}
            </div>
        ...
        {projects.map(proj => (
          <button key={proj.id} onClick={() => setSelectedProjectId(proj.id)} className="bg-card border border-border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow text-left">
            ...
            <div className="flex items-center justify-between">
              ...
              <span className="text-xs text-muted-foreground flex items-center gap-1"><Users size={12} /> {proj.teamIds.length}</span>
            </div>
          </button>
        ))}
      </div>
