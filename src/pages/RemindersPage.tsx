...
const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  ...
  failed: { label: 'Fallido', color: 'bg-destructive/10 text-[var(--rx-rose)] border-destructive/20', icon: XCircle },
  no_phone: { label: 'Sin teléfono', color: 'bg-[var(--rx-s2)] text-[var(--rx-t2)] border-[var(--rx-b1)]', icon: Phone },
};
    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="p-2 bg-primary/10 rounded-lg">
            <Bell className="text-[var(--rx-brand)]" size={22} />
          </div>
          <div>
            <h1 className="rx-page-title">Recordatorios</h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Fallidos', count: counts.failed, color: 'text-[var(--rx-rose)]' },
        ].map(s => (
          <div key={s.label} className="rx-panel">
            <p className="text-xs text-[var(--rx-t2)] font-medium">{s.label}</p>
        ...
        <TabsContent value={tab} className="mt-4">
          {loading ? (
            <div className="flex items-center justify-center py-12">
              <Loader2 className="animate-spin text-[var(--rx-brand)]" size={24} />
            </div>
          ) : filtered.length === 0 ? (
            <div className="text-center py-12 text-[var(--rx-t2)]">
                  ...
                  <div key={r.id} className="rx-panel">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {r.error_message && (
                          <div className="mt-2 flex items-start gap-1.5 text-xs text-[var(--rx-rose)] bg-destructive/5 rounded-md p-2">
                            <AlertTriangle size={12} className="shrink-0 mt-0.5" />
                            <span>{r.error_message}</span>
                            {(r.retry_count ?? 0) > 0 && (
                              <span className="ml-auto text-[var(--rx-t2)]">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        {(r.status === 'failed' || r.status === 'no_phone') && (
                          <button
                            ...
                            className="p-1.5 rounded-md text-[var(--rx-brand)] hover:bg-primary/10 transition-colors"
                            title="Reenviar"
                          >
                        ...
                        <button
                          ...
                          className="p-1.5 rounded-md text-[var(--rx-t2)] hover:text-[var(--rx-rose)] hover:bg-destructive/10 transition-colors"
                          title="Eliminar"
                        >
                          <Trash2 size={14} />
                        </button>
