-- Update schedule_appointment_reminders: 24h stays for email, 1h becomes voice call trigger.
-- Message body for 24h remains a full email-friendly text. Message body for 1h is a short
-- context string used only for logging (voice-outbound-call ignores it).
...
BEGIN
  ...
  -- 24h reminder needs email; 1h reminder needs phone (voice call)
  ...
  -- 24h: email only (only insert if we have an email)
  ...
  -- 1h: voice call (only insert if we have a phone)
